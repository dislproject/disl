...
if (option==4||option==5||option==6){stresses(option);}
else if(option==7||option==8||option==9){strains(option);}
colour_it();
}
...
for (int i=0; i<500; i++)
{
    for (int j=0; j<500; j++)
    {
        ...
        //store in grid for colouring
            if (opt==7){grid[i][j].val=grid[i][j].strain_net;}
            if (opt==8){grid[i][j].val=grid[i][j].strain_x;}
            if (opt==9){grid[i][j].val=grid[i][j].strain_y;}
    }
}
}
...
for (int i=0; i<500; i++)
{
    for (int j=0; j<500; j++)
    {
        ...
        //store in grid for colouring
            ...
            if (opt==4){grid[i][j].val=grid[i][j].stress_net;}
            if (opt==5){grid[i][j].val=grid[i][j].stress_x;}
            if (opt==6){grid[i][j].val=grid[i][j].stress_y;}
...
void Strain_map::colour_it()
{
    ...
    for (int m=0; m<k; m++)
    {
        if (old_max[m]>average+2*sd){max_s=old_max[m+1];}
    }
    ...
    for (int i=0; i<500; i++)
    {
        for(int j=0; j<500;j++)
        {
          ...
          //  qDebug()<<grid[i][j].val;
            ...
            if (grid[i][j].colour>10){grid[i][j].colour=10;}
            if (grid[i][j].colour<-10){grid[i][j].colour=-10;}
           //if (grid[i][j].colour!=0){qDebug()<<grid[i][j].val/bin_width<<grid[i][j].val<<grid[i][j].colour;}
        }
    }
