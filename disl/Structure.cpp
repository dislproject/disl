...
void Structure::Random_position_inplane(int line_type)
{
    ...
    if (line_type == 2)
    {
        ...
        if (pos_x > material_width)
        {
            pos_x -= material_width;
        }
...
{
    sep = 80;
    //  *INDENT-OFF* - "uncrustify" no reformat
    ...
    //  *INDENT-ON* - "uncrustify" no reformat
}
...
void Structure::Add_new_pair()
{
    ...
    if (rucks > 95)
    {
        Ruck_and_tuck(4, pos_x + 300, pos_y + 120 - (0.5 * int_layer_sep));
    }
    else
    {
        ...
        for (int i = 0; i < D.size(); i++)
        {
            ...
            if (T == 1)
            {
                ...
                for (int k = -num_cells; k <= num_cells; k++)
                {
                    ...
                    //if (j == 120)
                    //{
                    //    qDebug() << "adding strain due to" << i << "in cell" << k << xs + 300;
                    //}
