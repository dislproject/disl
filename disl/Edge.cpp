...
void Edge::Edge_strain( double B1, double x_in, double y_in ) {
    ...
    //if( rx >= 0. ) theta = 0.; // correction for atan2
    //else theta=2 * M_PI;
...
void Edge::Strain_aniso(double bx, double by, double x_in, double y_in, int type)
{
    ...
    if (type==3||type==4){ //prismatics
        // theta 1 - one arg
        if(rxa>=0.&&rya<=0.){theta1=3.5*M_PI;    //1
            theta2=-2.*M_PI;}
        if(rxa<0.&&rya<=0.){theta1=-0.5*M_PI;}   //2
        //	theta2=2.*M_PI;}
        if(rxa<0.&&rya>0.){theta1=-0.5*M_PI;     //3
            theta2=0.;}
        if(rxa>=0.&&rya>0.){theta1=-4.5*M_PI;    //4
            theta2=2.*M_PI;}}
    ...
    //  if(rxa<0.&&rya>0.&&pow(pow(rya,2),1/2)<rxa){theta1=theta2=-2*M_PI;}
    //  if(rxa<0.&&rya>0.&&pow(pow(rya,2),1/2)>rxa){theta1=theta2=2*M_PI;}
    //else if(rxa<0.&&rya>0.){
    //theta1=0.;//2*M_PI;
    //}
