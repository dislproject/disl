...
{
    if (d1<0||d2<0){qDebug()<<"referencing negative number... exiting";exit(0);}
    if (d1==D.size()||d2==D.size()){qDebug()<<"referencing item outside range of D list"<<d1<<d2<<D.size();exit(0);}
...
//        qDebug()<<its<<d1<<d2<<D[d1].group<<D[d2].group<<D[d1].B1<<D[d2].B1<<burg1_mag<<burg2_mag<<R<<D[d1].x<<D[d1].xd<<D[d2].x<<D[d2].xd<<x1<<x2<<D.size()<<net_burg_cons();
       ...
       //          qDebug()<<"(1)";
        ...
        //         qDebug()<<"(2)"<<ch;
         ...
         //              qDebug()<<"removing"<<d1<<d2;
                    if(d1>d2){D.removeAt(d1);D.removeAt(d2);} //ensure dislocations are removed in the right order
                    if(d1<d2){D.removeAt(d2);D.removeAt(d1);} //dislocation of the highest index is removed first.
                }
     ...
     //           qDebug()<<"interaction type 2"<<its;
      ...
      //              qDebug()<<"interaction type 2.2";
                    ...
                    if (ch==0){ //qDebug()<<"2.2 ch=0";
                        D[d2].B1=D[d2].by=new_burg;
                        D[d2].re_run=true;
                        if(D[d2].B1>0){D[d2].B=3;}
                        else {D[d2].B=4;}
                        D.removeAt(d1);}
                    else {
                      //  qDebug()<<"2.2 ch="<<ch;
  ...
  //          qDebug()<<"interaction type 5";
            ...
            if (burg1_mag==burg2_mag){
            if (d1>d2){D.removeAt(d1);D.removeAt(d2);}
            else if (d2>d1){D.removeAt(d2);D.removeAt(d1);}
   //         qDebug()<<"returning -2";
            return -2;
            }
            else if (burg1_mag!=burg2_mag)
            {
                ...
                D[d1].B1=newburg;
                D[d1].bx=D[d1].B1;
                if (newburg>0){D[d1].B=1;}
                else if (newburg<0){D[d1].B=2;}
                D.removeAt(d2);
   //             qDebug()<<"returning -1";
                return -1;
            }
...
bool Interaction::net_burg_cons()
{
 ...
 //       qDebug()<<"burg mag not conserved"<<net_burg;
    ...
    else {return true;}//qDebug()<<"pass"<<its+100<<"_"<<force_iteration; return true;}
}
...
//          print_group_stats(sorted_disl_list);
    ...
    for (int i=0;i<sorted_disl_list.size();i++){
        if (finished==false){
            ...
            finished=false;
            if (D[m].min==true){finished=true;}
            else {D[m].x+=material_width; }//qDebug()<<"adding to D[m].x"<<m;}
        }
    }
        ...
        {//qDebug()<<"finding minimum 3.2";
           ...
           //       qDebug()<<i<<a;
            ...
            {//qDebug()<<"finding minimum 3.2";
                int k=j+i;
                //   qDebug()<<i<<j<<k<<a;
                if (k>=sorted_list.size()){k-=sorted_list.size();}
...
int Interaction::Calc_interaction(int d1, int d2)
{
    ...
    if (d1<0||d2<0){qDebug()<<"referencing negative number... exiting";exit(0);}
    if (d1==D.size()||d2==D.size()){qDebug()<<"refernecing item outside range of D list"<<D.size();exit(0);}
    typedef Eigen::Matrix<double,3,1> vectorDim;
    vectorDim vectR;
    ...
    if(D[d1].B1<0){burg1_mag = (-1*D[d1].B1);}
    else{burg1_mag=D[d1].B1;}
    if(D[d2].B1<0){burg2_mag = (-1*D[d2].B1);}
    else{burg2_mag=D[d2].B1;}
    bool interact=false;
...

    {
        ...
        {  //if same sign add burgers vectors
            ...
            if(basals==1)
            {if (D[d1].B1>0){D[d1].B=1;}
                else{D[d1].B=2;}
                D[d1].bx=D[d1].B1;
                D[d1].by=0;
            }
            else if(basals==0){
                if (D[d1].B1<0){D[d1].B=4;}
                else{D[d1].B=3;}
                D[d1].by=D[d1].B1;
                D[d1].bx=0;
...
//                    int d3=d2+1;
                    ...
                    if(disl_group.size()==2&&(D[d1].group==D[d2].group)) //if next to each other then annihillate normally
                    {
                        ...
                        //            {qDebug()<<"net burg not conserved at point 3";exit(0);}
                        //qDebug()<<"checking interaction 1 done";
                        return -2;
                    }
                    else
                ...
                else {qDebug()<<"not prismatic or basal..."<<D[d1].B<<D[d2].B;}
            ...
            {   //if not equal add burg2 to burg1 remove dislocation 2
                ...
                else if(basals==1&&(D[d1].B==3||D[d2].B==3))
                {
                    ...
                    //                   {
                    //                       int multi=0;
                    //                       if (fabs(D[d1].B1)>fabs(D[d2].B1)){multi=int(fabs(D[d2].B1/basal_B));}
                    //                       if (fabs(D[d1].B1)<fabs(D[d2].B1)){multi=int(fabs(D[d1].B1/basal_B));}
                    //                       for(int i=0;i<multi;i++)
                    //                       {structure.Add_material_height();}
                    //                    }
                    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                //        if(net_burg_cons()==false)
                //        {qDebug()<<"net burg not conserved at point 6";exit(0);}
                //iteration loop starts on same number as currently as list will shift once.
                //qDebug()<<"checking interaction 1 done";
                return -1;
            }
    ...
    for (int d1=0; d1<D.size(); d1++)
    {
        for (int d2=0; d2<D.size();d2++)
        {
            if (d1!=d2)
            {
                ...
                {
                    double xsep=D[d1].xd-D[d2].xd;
                    if(xsep<0){xsep=xsep*-1;}

                    if(xsep<100)
                    {
                        double ysep=D[d1].y-D[d2].y;
                        //if (ysep<0){ysep=ysep*-1;}
...
{
    //qDebug()<<"climb";
    if(D[d].B1>0){D[d].y+=int_layer_sep;}
    else if(D[d].B1<0){D[d].y-=int_layer_sep;}
...
QList<int> Interaction::sort_list(QList<int> list)
{
    for (int j=0; j<list.size()-1;j++){
        ...
        for (int i=j+1; i<list.size();i++)
        {
            ...
            double x1=D[d1].x;
            double x2=D[d2].x;
            if (x1>x2){list.removeAt(j);list<<d1;j--;i=list.size();}
        }
...
int Interaction::check_dipole_size(int d1, int d2, int multi)
{
    ...
    for (int j=0; j<disl_list.size()-1;j++){
        ...
        for (int i=j+1; i<disl_list.size();i++)
        {
            ...
            double x1=D[dd1].x;
            double x2=D[dd2].x;
            if (x1>x2){disl_list.removeAt(j);disl_list<<dd1;j--;i=disl_list.size();}
        }
...

    {
        ...
        if(((min_3==d1)&&(max_4==d2))||((min_3==d2)&&(max_4==d1)))
        {
            ...
            if (min_3B==max_4B){
                ...
                if (min_3>max_4){D.removeAt(min_3);D.removeAt(max_4);return 2;}
                if (max_4>min_3){D.removeAt(max_4);D.removeAt(min_3);return 2;}
