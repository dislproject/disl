...
void Force::move_basals()
{
    ...
    for (int d = 0; d < D.size(); d++)
    {
        if (D[d].B == 1 || D[d].B == 2)
        {
            ...
            //put an upper limit on how far loops can move.
            if (move > 40)
            {
                move = 40;
            }
...
void Force::calc_total_diff_x(int d1)
{
    ...
    qDebug() << "force =" << force << ", 2nd diff =" << diff;
    confirm_diff(d1);
    if (d1 == 0)
    {
        qDebug() << "force =" << force << ", 2nd diff =" << diff;
...
double Force::calc_f_d1_d2(int d1, int d2, double x2)
{
    ...
    if (isnan(force) != 0)
    {
        qDebug() << "force is NAN" << d1 << d2 << D[d1].B1 << D[d1].x << D[d1].y << D[d2].B1 << D[d2].x << D[d2].y << dx << dy << x2;
        force = 0;
    }
...
void Force::inplane_forces()
{
    ...
    for (int d = 0; d < D.size(); d++)
    {
        ...
        if (D[d].y + 120 > material_height)
        {
            D[d].y -= material_height;
        }
        if (D[d].y < 0 + 120)
        {
            D[d].y += material_height;
