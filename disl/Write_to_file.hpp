...
/*  // write to log file
  ...
  if(T==1){a_file<<"Anisotropic"<<std::endl;}
  if(T==2){a_file<<"Isotropic"<<std::endl;}
  if(S==2){a_file<<"Circle"<<std::endl;}
  if(S==1){a_file<<"Square"<<std::endl;}
  ...
  if(T==1){c_file<<"Anisotropic ";}
  if(T==2){c_file<<"Isotropic ";}
...
void Write::Write_to_file()
{
   ...
   if (e_calc==1){a_file<<"Energy "<<En.En<<std::endl;   }
    a_file.close();
}
