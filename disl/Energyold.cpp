...
void Energy::Calc_energy()
{
    ...
    for(int k=0;(k<D.size());k++){
        if(D[k].image==false)
        {
            ...
            //qDebug()<<"1"<<En_self;
            // if(isnan(En_self)!=0){qDebug()<<En_self<<"not a number 1"; exit(0);}
        }
    }
    for(int l=0;l<ImgD.size();l++){
        if(ImgD[l].image==false)
        {
            ...
            //qDebug()<<"2"<<En_self;
            //if(isnan(En_self)!=0){qDebug()<<En_self<<"not a number 2"; exit(0);}
        }
    }
    ...
    for(int k=0;(k<D.size()-1);k++){ // calc En
        for(int l=k+1;(l<D.size());l++){
            ...
            //qDebug()<<"3"<<En.En;
            //if(isnan(En.En)!=0){qDebug()<<En.En<<k<<l<<D.size()<<D[k].x<<D[l].x<<D[k].y<<D[l].y<<"not a number 3"; exit(0);}
        }
    }
    for(int k=0;(k<D.size());k++){
        for(int l=0;l<ImgD.size();l++){
            if(D[k].image==false&&ImgD[l].image==true)
            {
                ...
                //qDebug()<<"4"<<En.En;
                //if(isnan(En.En)!=0){qDebug()<<En.En<<"not a number 4"; exit(0);}
            }
            else  if(D[k].image==true&&ImgD[l].image==false)
            {
                ...
                En+=(Iso_en_img(k,l));//*double(1.00e-11));
                //qDebug()<<"5"<<En.En;
                //if(isnan(En.En)!=0){qDebug()<<En.En<<"not a number 5"; exit(0);}
            }
        }
    }
    for(int k=0;(k<ImgD.size());k++){
        for(int l=0;l<ImgD.size();l++){
            if((ImgD[k].image==false&&ImgD[l].image==false)&&(k!=l))
            {
                ...
                //qDebug()<<"6"<<En.En;
                //if(isnan(En.En)!=0){qDebug()<<En.En<<k<<l<<ImgD[k].x<<ImgD[l].x<<ImgD[k].y<<ImgD[l].y<<"not a number 6"; exit(0);}
            }
        }
    }
